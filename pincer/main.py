...
class Pincer():
    ...
    format.
    """
    def __init__(self,source : str,padfilename : int = 3, report : bool = True):
        """
        Initialize a Pincer object.
        ...
        report : bool, optional
            If true, Pincer will periodically print processing updates to the 
            terminal.
    ...
    def import_formattedexcel(self,filepath : str):
        ...
        with pandas.ExcelFile(filepath) as xls:
            self.cellLabels = pandas.read_excel(
                xls,'CellLabels',header = 0, index_col=[0,1,2],dtype= str)
            self.traceIndex = pandas.read_excel(
                xls, 'TraceIndex',header=[0,1],index_col=[0,1,2],dtype= str)
            self.animalLabels = pandas.read_excel(
                xls, 'AnimalLabels',header=0,index_col=[0],dtype= str)
    ...
    def export_formattedexcel(self,filepath, *kwargs):
        ...
        *kwargs : TYPE
        ...
        """
        with pandas.ExcelWriter(filepath, *kwargs) as expfile:
    ...
    def process(self):
        ...
        """
        self._report('This may take a while...')
        self._process_all_analyses()
        self._process_secondary_measures()
        self._process_animalwise_measures()
        print('Done!')
            
    def _process_all_analyses(self):
        """
        Process all analyses in the analysis queue.

        This method iterates through the analysis queue, performs each analysis,
        ...
        """
        # This section runs all analyses and stores results
        self.missingfiles = []
        for analysis in self.analysis_queue.keys():
            self._report('Performing '+ type(self.analysis_queue[analysis]).__name__ + 'analysis')
            for index, row in self.traceIndex[analysis].iterrows():
                for name, code in row.items():
                    self._run_analysis(analysis,index,name,code)

    def _run_analysis(self,analysis,index,name,code):
        ...
        self._report('processing ',file)
        filepath = PurePath(self.source, Path(file))
        try:
            abf = PincerABF(str(filepath))
        except ValueError:
            self._report("missing file:"+file)
            print(f'MISSING {file}')
            self.missingfiles.append(file)
        else:
    ...
    def _process_secondary_measures(self):
        self._report('Calculating Secondary Measures')
        for op in self.secondary_ops_queue:
            features = [self.results[i[0]][i[1]] for i in op['inputIDs']]
            featureframe = pandas.concat(features,axis = 1)
            self.results['SecondaryOutputs',op['outputname']] = featureframe.apply(op['func'], axis = 1)
    
    def _process_animalwise_measures(self):
        self._report('Calculating Animalwise Measures!')
        self.animalresults = self.results.groupby(level=0,axis=0).mean()
    
    def _report(self,*args):
        """
        Print the provided arguments if the 'report' attribute is set to True.
    ...
    def cmbABFnm(self,daycode : str, index : str) -> str:
        """
